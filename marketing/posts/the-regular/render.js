// node render.js <scene> <outdir> [t1,t2,...]   → PNG frames at 24fps (or just the listed times)
const { chromium } = require('playwright'), fs = require('fs'), path = require('path');
(async () => {
  const [scene = 'bateen', out = 'frames', times] = process.argv.slice(2);
  ...
  const list = times ? times.split(',').map(Number) : Array.from({ length: Math.round((await p.evaluate(() => window.DURATION)) * 24) }, (_, i) => i / 24);
